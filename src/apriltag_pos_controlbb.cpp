...
bool goal_reached, moving;

void initGroundPlane() {
    // create a ground plane to prevent the motion planning from hitting ground
...
int main(int argc, char **argv)
{
    ...
    // hope for shorter execution time
    move_group.setNumPlanningAttempts(2);


    // now behavior based control starts
    goInitPos();


    while (ros::ok()) {
        // if there is no transformation to tag_0, move around to find
...
bool checkTR() {
    // todo: implement cartesian path that turns camera around and searches for apriltag
    ROS_INFO_STREAM("found no known transform for tag_0, moving robot to find tag_0!");
    ...
    std::vector<geometry_msgs::Pose> waypoints;
    // look around and find apriltag
    geometry_msgs::Pose pCurr = move_group.getCurrentPose().pose;
    tf::Quaternion q_rot, q_orig, q_new;
    geometry_msgs::Quaternion q_msg_new;
    q_rot = tf::createQuaternionFromRPY(0, 0, M_PI / 2.0);
    quaternionMsgToTF(pCurr.pose.orientation, q_orig);
    for (int i = 0; i < 4; i++) {
        q_new = q_rot * q_orig;
        q_new.normalize();
        quaternionTFToMsg(q_new, q_msg_new);
        pCurr.orientation = q_msg_new;
        waypoints.push_back(pCurr);
    }
    ...
    // execute with non-blocking call to be able to check for april tag
    success = (move_group.asyncExecute(trajectory_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
    ROS_INFO("Async-Executing Cartesian path %s", success ? "Succeeded" : "FAILED");

    // TODO: check if movement is complete -> stop and return false!
